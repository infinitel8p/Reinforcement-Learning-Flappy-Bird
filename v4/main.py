        ...
        for episode in itertools.count():
            ...
            if is_training:
                if episode_reward > best_reward:
                    ...
                    log_message = f"{datetime.now().strftime(DATE_FORMAT)}: New best reward {episode_reward} ({(
                ...
                if current_time - last_graph_update_time > timedelta(seconds=10):
                    self.save_graph(rewards_per_episode, epsilon_history, learning_rate_history)
                ...
                if len(memory) > self.mini_batch_size:
                    ...
                    for param_group in self.optimizer.param_groups:
                        param_group['lr'] = max(self.learning_rate_a - self.alpha_decay * episode, 0.1)
                    learning_rate_history.append(param_group['lr'])
                ...
                if episode % 100 == 0:
                    ...
                    log_message = f"{datetime.now().strftime(DATE_FORMAT)}: Episode {episode}, reward {
                        episode_reward:.3f}, epsilon {epsilon:.3f}, learning rate {current_learning_rate:.3f}"
                    # print(log_message)


    def save_graph(self, rewards_per_episode, epsilon_history, learning_rate_history):
        # Save plots
        fig = plt.figure(1)
        ...
        # Plot learning rate decay (Y-axis) vs episodes (X-axis)
        # plot on a 1 row x 2 col grid, at cell 3
        plt.subplot(133)
